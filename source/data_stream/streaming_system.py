...
def date_format(date):
    ...
    m = arr2.split(":")[1]
    s = arr2.split(":")[2]
    x = utcZ.localize(datetime(int(year),int(month),int(day),int(h),int(m),int(s)))
    #print("UTC"+year+":"+month+":"+day+":"+h+":"+m+":"+s)
    ict = x.astimezone(thaZ)
    fmt = '%Y-%m-%d_%H:%M:%S'
    ict = ict.strftime(fmt)
    return ict[2:]
    #print("ICT"+ict)
    #return date.replace("/","-").replace(" ","_")[2:]
...
for c in clusters:
    ...
    for i in content_file_cu[35:len(content_file_cu)-3]:
        line = i.split("</v></row>\n")[0].split("<row><v>")
        if len(line) == 2:
            key = date_format(line[0].split(" UTC / ")[0][8:])
            value = value_format(line[1])
